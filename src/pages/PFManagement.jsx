...
const PFManagement = () => {
  ...
  const [summary, setSummary] = useState({
    totalEmployees: 0,
    totalEmployeePfDeduction: 0,
    totalCompanyPfContribution: 0,
    totalCombinedPf: 0,
  });
  ...
  const fetchPfRecords = async (targetPage = page, search = searchTerm, dept = departmentFilter) => {
    setLoading(true);
    try {
      if (isEmployeeOnly) {
        ...
        const res = await api.get(`/pf?${params.toString()}`);
  ...
  const handleToggleMyPfOptIn = async (newVal) => {
    ...
    try {
      ...
      if (res?.success || res?.data) {
        ...
        fetchPfRecords(page, searchTerm, departmentFilter);
      } else {
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        ...
        const list = res?.data || (Array.isArray(res) ? res : []);
        const names = list.map((d) => d.name || d.departmentName).filter(Boolean);
  ...
  useEffect(() => {
    fetchPfRecords(1, searchTerm, departmentFilter);
  }, [isEmployeeOnly]);
  ...
  const departments = ["All", ...new Set([...departmentList, ...records.map((r) => r.department).filter(Boolean)])];

  // KPI Calculations (Server summary or fallback)
  const totalEmployeesCount = summary?.totalEmployees ?? totalRecords;
  const totalEmployeePfDeduction = summary?.totalEmployeePfDeduction ?? 0;
  const totalCompanyPfContribution = summary?.totalCompanyPfContribution ?? 0;
  const totalCombinedPf = summary?.totalCombinedPf ?? 0;
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-2xl font-extrabold text-gray-900 mt-1">{totalEmployeesCount}</h3>
            </div>
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setSearchTerm(val);
              setPage(1);
              fetchPfRecords(1, val, departmentFilter);
            }}
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setPage(1);
                fetchPfRecords(1, searchTerm, val);
              }}
          ...
          <>
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/50">
              <div className="text-xs text-gray-500 font-medium">
                Showing <span className="font-semibold text-gray-900">{totalRecords > 0 ? (page - 1) * limit + 1 : 0}</span> to{" "}
                <span className="font-semibold text-gray-900">{Math.min(page * limit, totalRecords)}</span> of{" "}
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() => {
                    ...
                    setPage(newPage);
                    fetchPfRecords(newPage, searchTerm, departmentFilter);
                  }}
                  className="px-3.5 py-1.5 rounded-xl border border-gray-300 text-xs font-semibold text-gray-700 bg-white hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed transition shadow-sm cursor-pointer"
                >
                  Previous
                </button>
